...
hermes.gpx = (() => {
    ...
    function formatDuration(seconds) {
        const h = Math.floor(seconds / 3600).toString().padStart(2, '0'); const m = Math.floor((seconds % 3600) / 60).toString().padStart(2, '0'); const s = Math.floor(seconds % 60).toString().padStart(2, '0'); return `${h}:${m}:${s}`;
    }
    function formatPace(seconds, distanceKm) {
        if (distanceKm === 0) return '0′00″'; const pace = seconds / distanceKm; const min = Math.floor(pace / 60); const sec = Math.floor(pace % 60).toString().padStart(2, '0'); return `${min}′${sec}″`;
    }
    function encode(val) {
        val = val < 0 ? ~(val << 1) : (val << 1); let result = ''; while (val >= 0x20) { result += String.fromCharCode((0x20 | (val & 0x1f)) + 63); val >>= 5; } result += String.fromCharCode(val + 63); return result;
    }
    ...
    function addTrackLayer(track) {
        ...
        state.map.addSource(sourceId, { type: 'geojson', data: { type: 'Feature', geometry: { type: 'LineString', coordinates: track.points } } });
        state.map.addLayer({
    ...
    function initDragAndDrop() {
        ...
        mapContainer.addEventListener('dragenter', (e) => { e.preventDefault(); dragCounter++; mapContainer.classList.add('drag-over'); });
        mapContainer.addEventListener('dragover', (e) => { e.preventDefault(); });
        mapContainer.addEventListener('dragleave', (e) => { e.preventDefault(); dragCounter--; if (dragCounter === 0) { mapContainer.classList.remove('drag-over'); } });
