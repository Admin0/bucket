...
const getTextColorByBackgroundColor = (hexColor) => {
  ...
  // 색상 선택
 if (luma < 200) {
  document.documentElement.style.setProperty("--color-dark-tooltip", "rgba(255, 255, 255, .75)");
 } else {
  document.documentElement.style.setProperty("--color-dark-tooltip", "rgba(0, 0, 0, .85)");
 }
  // const result = luma < 127.5 ? 'white' : 'black'
  // console.log(luma)
  // return result // 글자색이
}
getTextColorByBackgroundColor(color.material_a200[color.i])
